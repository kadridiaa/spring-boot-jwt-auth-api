...
@DisplayName("ServiceBController - Tests d'accès avec permissions ACCESS_B")
class ServiceBControllerTest {
    ...
    @Test
    @DisplayName("GET /hello - header X-Permissions: ACCESS_B → 200 OK")
    void hello_withXPermissionsAccessB_returns200() throws Exception {
        mockMvc.perform(get("/api/service-b/hello")
                        .header("X-Permissions", "ACCESS_B"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Service B")));
    }

    @Test
    @DisplayName("GET /hello - header X-Permissions: ACCESS_A seulement → 403 Forbidden")
    void hello_withXPermissionsAccessAOnly_returns403() throws Exception {
        mockMvc.perform(get("/api/service-b/hello")
                        .header("X-Permissions", "ACCESS_A"))
                .andExpect(status().isForbidden())
                .andExpect(content().string(containsString("ACCESS_B")));
    }
    ...
    @Test
    @DisplayName("GET /hello - JWT avec ACCESS_B (pas de X-Permissions) → 200 OK")
    void hello_withJwtContainingAccessB_returns200() throws Exception {
    ...
    @Test
    @DisplayName("GET /hello - JWT sans ACCESS_B → 403 Forbidden")
    void hello_withJwtMissingAccessB_returns403() throws Exception {
        String jwt = buildJwt(List.of("ACCESS_A"));
